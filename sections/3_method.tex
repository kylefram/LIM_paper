...
We denote the loss function $L(x(\lambda),y(\lambda))$ for arbitrary target $x$ (to be approximate) and simulation $y$ (to be solved for) defined over the domain $\lambda \in \omega$. 
...
We minimize loss by selection of $k$ representing our $N$ 'active' filters (which incorporate the intrinsic sensitivity of a grey-scale sensor) each having transmission functions $f_k(\lambda)$ (thus comprising an $N\times1$ vector of functions), and $\boldsymbol{M}$, a $3\times N$ color correction matrix:
...
In the naive solution, we find the optimal filter combination for every subset of $M$ filters (where $M$ is small) drawn from the $N$ filter set by selecting the combination with the lowest loss, considering all combinations of filters.
We plot the errors across all combinations for M=5:
%% Kyle N=5?

\begin{center}
    \centering
    \includegraphics[width=1.05\linewidth]{media/all_errors.png}
    \captionof{figure}{MSE Error distribution across all filter combinations for M=5}
\end{center}
...
This method effectively creates one $3\times N$ matrix $\boldsymbol{M}$ and iteratively reduces weights that can be discarded with minimal loss, until reaching $M$ nonzero entries.
