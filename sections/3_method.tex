...
We will choose filters drawn from a set of {\color{red} $M$} Rosco Supergel filters that we measure in our lab. 
...
We minimize loss by selection of $k$ representing our $k$ `active' filters (which incorporate the intrinsic sensitivity of a grey-scale sensor) each having transmission functions $\underline{f}_k(\lambda)$ (thus comprising a $k$ length vector of functions), and $\boldsymbol{M}$, a $3\times k$ color correction matrix. 
...
In Figure {\color{red} 2} we plot the errors across all sets of 5 filters (multiplied by the camera sensitivity) from the worst to best performing quintuple. 
We compute the Moore-Penrose pseudo-inverse for each set of filters. 
Our implementation is parallelized over 8 NVIDIA L4 GPUs, with a combined pool of up to 192GB VRAM, enabling us to compute brute-force searches of $k=5$ approximately 2 minutes. {\color{redd} Clearly, in terms of MSE there is a large difference between the best and worst 5-filter filter set.}
%% Kyle N=5?
...
This method initially creates one $3\times M$ matrix $\boldsymbol{M}$ {\color{red} (i.e. a single combination using the whole set of Roscoe filters)}and iteratively {\color{red} de}selects until reaching $k$ filters and thus our final $3 \times k$ matrix of weights. 
...
However, we note that the {\color{red} improved} MSE result for the brute force selection methodology.
