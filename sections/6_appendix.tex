...
Both routines were run on NVIDIA GPU environment with the Brute Force run pruned to cosine similarity of .95 and able to utilize 6 GPUs. 
Conversely, the PCA algorithm used one GPU for calculation and 6 GPUs only for the latter steps. 
...
\begin{table}[htbp]
    ...
    \bottomrule
    \end{tabular}
    \caption{Timing Comparison: PCA vs. Brute Force}
    \label{tab:timing_comparison}

\end{table}
\newpage
We also include the full loss and $\Delta E_{2000}$ results: 
...
\begin{table}[htbp]
    ...
    \bottomrule
    \end{tabular}
    \caption{Minimum MSE and $\Delta E$ Comparison: PCA vs. Brute Force, up to $N=6$, $N_2=2$}
