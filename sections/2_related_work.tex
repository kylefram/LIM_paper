...
This method achieves {\color{red} optimality} only when we approach Luther Condition compliant capture \cite{ives1915transformation}, specifically that our spectral sensitivity functions be linearly related to our chosen $XYZ$ color matching function. 
...
The function $\rho'_c(\lambda)$ denotes a single estimated camera sensor where $c\in \{R,G,B\}$,
$\rho'_c(\lambda)=\mathbf{M}_c\underline{f}(\lambda)$, where ${M}_c$ denotes the corresponding row of $\mathbf{M}$.
In the optimizations that follows we will solve for the R, G and B sensors that best approximate the actual X, Y and Z color matching function sensitivities.
