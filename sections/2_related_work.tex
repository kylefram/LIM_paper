...
Let $\underline{f}_k(\lambda)$ and $\boldsymbol{M}_c$ respectively denote $k$ filter functions multiplied by the camera sensor response and the $c$'th row of a $3 \times k$ matrix that linearly combines these filter functions (e.g. to best approximate the XYZ colour matching functions). 
...
\begin{equation}
{\rho}_{c,k}=\int_\omega \mathbf{M}_c\underline{f}_k(\lambda)E(\lambda)S(\lambda)d\lambda
\end{equation}
