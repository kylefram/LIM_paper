...
%Kyle, removed as it is not needed (repetitious).
%However, t
...
{\color{red} for a } color correction matrix $\boldsymbol{M}$ {\color{red} best matching the set of filtered camera sensitivities to the XYZ colour matching functions}. 
...
In the discussion that follows $\rho'_c(\lambda)$ denotes a single estimated camera sensor where $c\in \{R,G,B\}$. In the optimisations that follows we will solve for the R, G and B sensors that best approximate the actual X, Y and Z color matching function sensitivities.

%%%%%% end of alternate math 

We define our camera's response for color channel $c\in \{R,G,B\}$ as the {\color{red} integral} dot product between the corresponding row of $\boldsymbol{M}$ (defined $M_c$) and our selected filters' transmission functions $f_k(\lambda)$.
