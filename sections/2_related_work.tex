...
We follow the work of Gao et al. \cite{gao2024color} in matching our simulated camera's spectral sensitivity function to the human eye by optimizing for a color correction matrix $\boldsymbol{M}$ best matching the set of filtered camera sensitivities to the XYZ colour matching functions. 
...
Let $\underline{f}_k(\lambda)$ and $\boldsymbol{M}$ respectively denote $k$ filter functions multiplied by the camera sensor response and a $3 \times k$ matrix that linearly combines these filter functions (e.g. to best approximate the XYZ colour matching functions). Then the 3-component filtered colour response, $\underline{\rho}'$ is written as:

\begin{equation}
\underline{\rho}'_k=\int_\omega \mathbf{M}\underline{f}_k(\lambda)E(\lambda)S(\lambda)d\lambda
\end{equation}
